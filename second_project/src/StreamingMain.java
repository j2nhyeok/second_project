...
public class StreamingMain extends JFrame {
	...
	private PlayListDaoImp dao = new PlayListDaoImp();
	...
	public StreamingMain(String user_id) throws SQLException {
		...
		sl_total.putConstraint(SpringLayout.NORTH, rank_panel, 74, SpringLayout.NORTH, total);
		...
		JLabel lblNewLabel_2 = new JLabel(dao.findUsersName(conn, user_id) + "님 환영합니다.");
		...
		sl_rank_panel.putConstraint(SpringLayout.NORTH, rankPanel1, 29, SpringLayout.NORTH, rank_panel);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (list.size() == 0) {
					...
					for (int i = 0; i < list.size(); i++) { // 플레이리스트 테이블에 데이터 입력
						try {
							dao.SetSaveCnt(conn, list.get(i));
						} catch (SQLException e1) {
							e1.printStackTrace();
						}
						try {
							dao.insertPL(conn,user_id, list.get(i));
						} catch (SQLException e1) {
							e1.printStackTrace();
						}
					}
					for(int i = 0 ; i <10; i++) {
						try {
						dao.updateRank(conn, dao.printMusicId(conn, i), i + 1);
						} catch (SQLException e1) {
							e1.printStackTrace();
						}
					}
	...
	sl_rank_panel.putConstraint(SpringLayout.NORTH,btnNewButton,2,SpringLayout.SOUTH,rankPanel10);sl_rank_panel.putConstraint(SpringLayout.EAST,btnNewButton,-10,SpringLayout.EAST,rank_panel);rank_panel.add(btnNewButton);

	SpringLayout sl_panel_1 = new SpringLayout();panel_1.setLayout(sl_panel_1);

	JLabel lblNewLabel_1 = new JLabel(
			"광고 넣을거임! 내맘임 ㅋ");sl_panel_1.putConstraint(SpringLayout.NORTH,lblNewLabel_1,4,SpringLayout.NORTH,panel_1);sl_panel_1.putConstraint(SpringLayout.WEST,lblNewLabel_1,4,SpringLayout.WEST,panel_1);sl_panel_1.putConstraint(SpringLayout.SOUTH,lblNewLabel_1,67,SpringLayout.NORTH,panel_1);sl_panel_1.putConstraint(SpringLayout.EAST,lblNewLabel_1,158,SpringLayout.WEST,panel_1);panel_1.add(lblNewLabel_1);



	setSize(638,500);

	setDefaultCloseOperation(EXIT_ON_CLOSE);setLocationRelativeTo(null);// 창이 가운데 나오게
	setVisible(true);setResizable(false);// 창의 크기를 변경하지 못하게
	}
