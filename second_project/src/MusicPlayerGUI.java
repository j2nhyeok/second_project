...
public class MusicPlayerGUI extends JFrame {
	...
	private MusicPlayer mp = new MusicPlayer(dao.getPlayingIDs());

	public MusicPlayerGUI(String user_id) throws SQLException {
		this.user_id = user_id;
		setTitle(dao.findUsersName(conn, user_id) + "님의 플레이 리스트");
		setResizable(false);
		...
		dao.printPlaylist(conn, user_id, data);
		...
		springLayout.putConstraint(SpringLayout.NORTH, scrollPane, 72, SpringLayout.NORTH, getContentPane());
		...
		springLayout.putConstraint(SpringLayout.NORTH, playBtn, 0, SpringLayout.NORTH, btnNewButton_3);
		playBtn.setForeground(Color.BLACK);
		getContentPane().add(playBtn);

		JButton deleteBtn = new JButton("삭제");
		springLayout.putConstraint(SpringLayout.NORTH, deleteBtn, 10, SpringLayout.SOUTH, scrollPane);
		springLayout.putConstraint(SpringLayout.EAST, deleteBtn, 0, SpringLayout.EAST, scrollPane);
		getContentPane().add(deleteBtn);

		JLabel lblNewLabel_2 = new JLabel("플레이리스트");
		lblNewLabel_2.setForeground(Color.CYAN);
		lblNewLabel_2.setFont(new Font("12롯데마트드림Medium", Font.ITALIC, 26));
		springLayout.putConstraint(SpringLayout.WEST, lblNewLabel_2, 50, SpringLayout.EAST, panel);
		springLayout.putConstraint(SpringLayout.SOUTH, lblNewLabel_2, -18, SpringLayout.NORTH, scrollPane);
		getContentPane().add(lblNewLabel_2);

		JCheckBox allSelectBtn = new JCheckBox("전체 선택");
		springLayout.putConstraint(SpringLayout.NORTH, allSelectBtn, 10, SpringLayout.SOUTH, scrollPane);
		springLayout.putConstraint(SpringLayout.EAST, allSelectBtn, -170, SpringLayout.EAST, getContentPane());
		...
		deleteBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					dao.DeletePL(conn, user_id, check);
